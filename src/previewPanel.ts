...
import { getLanguageColorRules } from "./languageColor";
import { getReleaseNotesHTML } from "./releaseNotes";
...
export function createPreviewPanel(isBeta: boolean) {
  try {
    ...
    const releaseNotesHtml = getReleaseNotesHTML();
...
function handleWebviewMessage(message: any, panel: vscode.WebviewPanel) {
  switch (message.type) {
    ...
    case "getLanguageRules":
      const rules = getLanguageColorRules(message.language);
...
async function updateSyntaxColor(
  ...
  const textMateRules = (tokenColorCustomizations as any).textMateRules || [];

  // Find or create the rule
  const existingRuleIndex = textMateRules.findIndex((rule: any) =>
...
async function exportTheme(theme: any) {
...
async function createNewTheme(name: string) {
  if (!name || name.trim() === "") {
  ...
  try {
    ...
    vscode.window.showInformationMessage(
      `✅ Theme "${name}" created successfully at:\n${themeFilePath}`,
    );
  } catch (error) {
...
function getThemeStudioHtml(isBeta: boolean, releaseNotesHtml: string) {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
